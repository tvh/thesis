...
For this reason the Skeletons cannot be shared between the 2.
...
\end{lstlisting}
This means it is still possible to use typed expressions in places where untyped expressions are exprected.
...
\begin{itemize}[noitemsep]
 ...
 \item for: \lstinline[breaklines=true]!for <ty> <var> in <val1> ( to | downto ) <val2> { <instructions> }!
\end{itemize}
...
src/LLVM/General/Quote/Base.hs:681:54:
    Could not deduce (ToConstant a0) arising from a use of 'toConstant'
